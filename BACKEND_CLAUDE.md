...
| File | Role |
|------|------|
| `full_metrics_viewer.py` | **Main entry point.** Launches WebSocket/REST connections, instantiates all sub-engines, runs Rich terminal dashboard at 4 FPS, writes snapshot files, starts embedded API server. Supports `--headless` for server deployment (no Rich UI, logs to stdout). |
| `ws_connectors.py` | `BinanceConnector`, `BybitConnector`, `OKXConnector`, and `MultiExchangeConnector` — WebSocket streams for depth, aggTrade, forceOrder/liquidations, markPrice. All three exchanges active. All error paths log via `logger` (Sprint 1 H2-lite). OKX connector also writes to `ws_connectors.log` via dedicated file logger (`_ws_file_logger`) for diagnostics visible outside Rich UI. |
...
| `oi_poller.py` | `MultiExchangeOIPoller` + `OIPollerThread` — polls OI from Binance, Bybit, OKX every 5s per symbol (BTC, ETH, SOL). Aggregates across exchanges, delivers per-symbol updates via callback. All OI in base asset. |
...
| `embedded_api.py` | **Embedded** FastAPI server (v3.0.0). Runs as daemon thread inside viewer. Shares per-symbol `ob_buffers` dict by direct Python reference (BTC, ETH, SOL). Receives `EngineManager` for V3 zone endpoints. Receives `FullMetricsProcessor` for `/market_data` and `/trader_ratios`. Receives per-symbol `_liq_events` deques for `/liq_events`. Includes `ResponseCache` with LRU eviction (`MAX_CACHE_ENTRIES=500`) for concurrent-user scaling. All endpoints (V1–V3 + market data + events) available. **This is now the only API server needed.** |
...
| Stream | Data | Used By |
|--------|------|---------|
| `{sym}usdt@depth@500ms` (×3) | Orderbook diffs (500ms throttle) | Per-symbol `OrderbookReconstructor` → `OrderbookAccumulator` → 30s OB heatmap frames. BTC, ETH, SOL. |
...
**OKX Keepalive:** Protocol-level WebSocket pings are **disabled** (`ping_interval=None`). OKX requires application-level text-based keepalive: a concurrent `_okx_keepalive()` async task sends text `"ping"` when no data has been received for 15 seconds. Responses of text `"pong"` and incoming text `"ping"` (responded to with text `"pong"`) are handled before JSON parsing.

**OKX File Logger:** A dedicated file logger (`_ws_file_logger`, writes to `poc/ws_connectors.log`) records OKX connection lifecycle, subscription confirmations/errors, keepalive activity, 30-second data flow summaries, disconnections with close codes, and exceptions with tracebacks. This logger has `propagate=False` so it does not duplicate into the Rich UI. Use `tail -f poc/ws_connectors.log` to monitor OKX connectivity while the Rich dashboard is running.
...
### WebSocket Reconnect & Backoff (all connectors)

All three connectors share the same exponential backoff constants (defined at module level in `ws_connectors.py`):

| Constant | Value | Purpose |
|----------|-------|---------|
| `_BACKOFF_INITIAL` | 1.0s | Initial reconnect delay |
| `_BACKOFF_MAX` | 30.0s | Maximum reconnect delay cap |
| `_BACKOFF_JITTER_LOW` | 0.8 | Lower jitter multiplier |
| `_BACKOFF_JITTER_HIGH` | 1.2 | Upper jitter multiplier |
| `_REPEATED_ERROR_THRESHOLD` | 5 | Downgrade to DEBUG logging after this many consecutive identical errors |

**Backoff formula:** `delay = min(1.0 * 2^(attempt-1), 30.0) * random(0.8, 1.2)`

**Reset trigger:** Backoff resets to 0 after the first successful `on_message()` callback on a new connection (not on TCP connect). This prevents reset-on-connect-then-immediate-drop scenarios.

**Binance** uses per-stream backoff tracking (`_backoff[stream_name]`). **OKX** and **Bybit** use global `_attempt`/`_last_err` tracking.

### REST Polling

**Binance proxy routing:** All Binance REST calls (OI + trader ratios) route through `proxy.tradenet.org` to avoid IP bans on the backend server. Base URLs: `BINANCE_BASE` in `oi_poller.py` and `BINANCE_FAPI_BASE` in `rest_pollers.py`, both set to `"https://proxy.tradenet.org/futures"`. The proxy strips the `/futures` prefix and forwards to `fapi.binance.com`. Endpoint paths must NOT include a `/futures` prefix — the base URL already provides it. Bybit and OKX URLs are direct (not proxied).

#### Open Interest (via `MultiExchangeOIPoller` in `oi_poller.py`)

Polls every 5 seconds for BTC, ETH, SOL. 3 symbols × 3 exchanges = 9 requests per cycle (108 req/min).
...
All Binance REST calls route through the proxy (`BINANCE_FAPI_BASE = "https://proxy.tradenet.org/futures"`). Endpoint paths are relative to this base — do NOT include a `/futures` prefix in paths since the base already provides the proxy routing prefix.

| Endpoint Path (relative to BINANCE_FAPI_BASE) | Interval | Data |
|----------|----------|------|
| `/data/topLongShortAccountRatio` | 60s | Top trader account long/short ratio |
| `/data/topLongShortPositionRatio` | 60s | Top trader position long/short ratio |
| `/data/globalLongShortAccountRatio` | 60s | Global account long/short ratio |

## CLI Arguments (`full_metrics_viewer.py`)

```
python full_metrics_viewer.py [--api] [--api-host HOST] [--api-port PORT] [--headless]
```

| Argument | Default | Purpose |
|----------|---------|---------|
| `--api` | off | Enable embedded API server (FastAPI on port 8899) |
| `--api-host` | `127.0.0.1` | API bind address. Use `0.0.0.0` for production/remote access |
| `--api-port` | `8899` | API listen port |
| `--headless` | off | Disable Rich terminal UI for server deployment. Logs to stdout via `logging.basicConfig` (INFO level). Main loop polls at 1s instead of 250ms display refresh. All other functionality (WS, pollers, API, snapshots, calibration) runs identically. |

**Typical deployment command:** `python full_metrics_viewer.py --api --api-host 0.0.0.0 --headless`
...
```python
HeatmapConfig(symbol={SYM}, steps={from SYMBOL_CONFIGS}, decay=0.999, buffer=0.002,
              tape_weight=0.35, projection_weight=0.65)
# Also: cluster_radius_pct=0.005, min_notional_usd=10000.0, max_pools_per_side=20
# Note: HeatmapConfig default decay is 0.998, but viewer overrides to 0.999 at instantiation.
```
...
```python
symbols=["BTC", "ETH", "SOL"], poll_interval=5.0
...
### Per-symbol OI History Ring Buffer
```python
self._oi_history_lock = threading.Lock()
self._oi_history: Dict[str, deque] = {sym: deque(maxlen=17280) for sym in SYMBOL_CONFIGS}
# 24 hours at 5s intervals = 17280 entries per symbol (~276KB each)
# Each entry: (timestamp_ms: int, aggregated_oi: float)
# Appended in _on_oi_update() under _oi_history_lock
# Read by /oi_history API endpoint (copy-on-read under lock, bisect range filter)
# No disk persistence — data starts accumulating from process start
```
...
**Applies to projection buckets only.** Tape (observed liquidation flow) hard-deletes on sweep because tape represents consumed liquidity — semantically different from projected inventory. However, tape buckets have a grace period (`SWEEP_GRACE_SECONDS = 120`) — buckets younger than 2 minutes are immune to sweep, allowing events at the current price to accumulate before being eligible for consumption.
...
|----------|-------|----------|---------|
| `MAX_OI_REDUCTION_PER_MINUTE` | 0.10 (10%) | 0.25 (25%) | Maximum fraction of projections removed per minute on OI drop. A 2-minute decline now removes ~19% instead of 44%. Reduced further to prevent natural OI fluctuations from compounding with decay to kill levels prematurely. |

Reduction formula: `close_factor = min(abs(oi_delta) / total_oi, MAX_OI_REDUCTION_PER_MINUTE)` — scales proportionally to the magnitude of the OI drop relative to total OI, capped at 10% per minute. Falls back to `abs(oi_delta) * src_price / total_projected` when total OI is unavailable.

### Bucket Alignment Snap

When `_project_side()` computes a bucket price for a new inference, small price shifts (1 step) between minutes can cause the inference to land in an adjacent bucket instead of reinforcing an existing one. This creates two weak buckets instead of one strong one, accelerating decay-driven disappearance.

**Fix:** Before creating a new bucket, `_project_side()` checks if an existing bucket exists within ±1 step (`near_lo` / `near_hi`). If found, the inference is snapped to the existing bucket. Priority: exact match > lower neighbor > upper neighbor. This ensures that levels accumulate strength over time instead of fragmenting across adjacent buckets.
...
- **`get_api_response_display()`** — display wrapper that calls `get_api_response()` then applies cluster-level `COMBINED_ZONE_BOOST` and adds `source` field to each pool (`"tape"`, `"inference"`, or `"combined"`). Used for V2 snapshot `long_levels`/`short_levels` and BTC terminal zone tracker. Stores cluster boost stats in `_last_cluster_boost_stats` for the next `get_heatmap_display()` → debug log cycle.
...
### Visibility Floor (Display-only)

Post-normalization lift applied during V1/V2 snapshot grid construction in `full_metrics_viewer.py`. Prevents buckets with significant USD notional from being crushed to invisible intensity by p99 normalization when a few outlier buckets dominate.

| Constant | Value | Purpose |
|----------|-------|---------|
| `VISIBILITY_FLOOR_THRESHOLD` | `{"BTC": 25000, "ETH": 10000, "SOL": 5000}` | USD notional above which a bucket gets the floor applied |
| `VISIBILITY_FLOOR_MIN_U8` | `25` | Minimum u8 intensity (~10% of 255) for qualifying buckets |
| `VISIBILITY_FLOOR_MIN_FLOAT` | `≈0.098` | Float equivalent (`25 / 255.0`) |

Applied at the grid level (u8 intensity arrays in snapshot JSON) — never modifies the raw heatmap data used by calibrator or zone logic.
...
```
MultiExchangeOIPoller (oi_poller.py) polls every 5s:
  ...
  → on_oi_update(symbol_short, aggregated_oi, per_exchange_dict)
    → full_metrics_viewer._on_oi_update()
      ...
      → self._oi_history[sym].append((ts_ms, aggregated_oi)) (for /oi_history API endpoint)
...
### Path E: Liquidation Event → Event Buffer → /liq_events API

```
_process_liquidations() (after engine_manager.on_force_order succeeds)
  → builds event dict {ts_ms, symbol, side, price, qty, notional_usd, exchange, leverage_tier}
  → acquires _liq_events_lock
  → appends to _liq_events[symbol] deque (maxlen=12000 per symbol)
  → releases lock

/liq_events endpoint (embedded_api.py)
  → acquires liq_events_lock
  → copies events from deque for requested symbol
  → releases lock
  → filters by since_ts, applies limit cap (max 5000)
  → returns {symbol, events[], count, oldest_available_ts, newest_ts, ts_unit: "ms"}
```
...
|--------|-------------|-------|--------|-------------|-----------|
| GET | `/health` | `/v1/health` | — | In-memory state (processor, OI poller, per-symbol snapshots, exchange status) | none |
| GET | `/oi` | — | `symbol` (BTC/ETH/SOL) | `OIPollerThread` → `OISnapshot` (in-memory) | 5s |
| GET | `/oi_history` | — | `symbol`, `from` (ms, required), `to` (ms, required) | Per-symbol `_oi_history` deque (in-memory, maxlen=17280) | 5s |
| GET | `/market_data` | `/v2/market_data` | `symbol` | Processor prices/funding + OI poller | 5s |
| GET | `/trader_ratios` | `/v2/trader_ratios` | `symbol` (BTC only, rejects others with 400) | `Processor.state` ratio fields (Binance REST) | 5s |
| GET | `/liq_heatmap` | `/v1/liq_heatmap` | `symbol` | `liq_api_snapshot_{SYM}.json` (5s refresh, per-symbol) | 5s |
| GET | `/liq_heatmap_history` | `/v1/liq_heatmap_history` | `symbol`, `minutes` (5-720), `stride` (1-30) | REMOVED — always returns 503. V1 binary files deleted on startup. | N/A |
...
| GET | `/liq_events` | `/v2/liq_events` | `symbol`, `limit` (1-5000, default 500), `since_ts` (ms) | Per-symbol `_liq_events` deque (in-memory, maxlen=12000) | none |
...
**Note:** Binary format responses (`format=bin`) for orderbook endpoints bypass the `ResponseCache` and are always generated fresh. `/liq_events` also bypasses the cache (always fresh).

**Input validation (Sprint 1 C5):** Orderbook endpoints validate user-provided `step` (must be > 0, returns 400) and `range_pct` (must be > 0 and ≤ 1.0, returns 400) before any grid computation. Grid size is capped at `MAX_GRID_BUCKETS = 10000` to prevent DoS via tiny step + wide range (returns 400 if exceeded).
...
|----------------|-----|-----------|
| Live data (`/oi`, `/market_data`, `/trader_ratios`, `/liq_heatmap`, `/liq_heatmap_v2`, `/liq_stats`, `/liq_zones`, `/liq_zones_summary`, `/liq_zones_heatmap`, `/orderbook_heatmap`) | **5 seconds** | Snapshot data updates every 60s (liq), 30s (OB), or 5s (OI); 5s is the minimum allowed by CLAUDE.md rules |
...
| Event streaming (`/liq_events`) | **none** | Cache bypassed; always returns fresh event data from in-memory deque |
...
### RESOLVED: V2 API collapsed all buckets into 1 level per side — greedy clustering chain-merge
- **Root cause:** `_cluster_zones()` in `liq_heatmap.py` compared each bucket to the *last added* bucket in the cluster (`cluster_prices[-1]`). With $20 step buckets and a ±$500 radius (`cluster_radius_pct=0.005` at BTC $100k), every consecutive bucket was within range of its neighbor. The greedy algorithm chained: A→B ($20 apart), B→C ($20 apart), ..., merging the entire side into one cluster with `bucket_count=116+`.
- **Impact:** `/liq_heatmap_v2` returned 1 `long_levels` entry and 1 `short_levels` entry despite 100+ raw buckets existing internally. Historical snapshots showed correct multi-level distribution (672 price levels) because they were generated before the bucket population grew large enough to chain-merge.
- **Fix:** Changed clustering to compare each bucket against the cluster's *seed price* (first bucket), not the last added. `buckets[j][0] - seed_price <= cluster_radius` instead of `buckets[j][0] - cluster_prices[-1] <= cluster_radius`. This caps each cluster at the intended ±0.5% diameter instead of growing unboundedly via chaining.

### RESOLVED: Tape accumulated 0 buckets — force order events swept within 1 minute of creation
- **Root cause:** Force order events arrive at the current market price (that's where the liquidation occurred). The tape records them at that price bucket. On the next `sweep()` call (every minute with OHLC high/low), the minute's high/low encompasses the liquidation price, so `high >= bucket` (shorts) or `low <= bucket` (longs) is immediately true. Every tape bucket was created and swept within 0-1 minutes, leaving the tape permanently empty.
- **Impact:** `liq_tape_BTC.jsonl` showed `forceOrder` entries proving events arrived, but `get_heatmap()` always returned empty dicts. The tape layer contributed zero signal to the combined heatmap, and `source` was always `"inference"` (never `"tape"` or `"combined"`).
- **Fix:** Added `SWEEP_GRACE_SECONDS = 120` (2 minutes) to `LiquidationTape`. Sweep now skips buckets where `last_ts >= grace_cutoff` (`ts_now - 120`). Buckets accumulate for at least 2 minutes before becoming eligible for sweep. This allows tape events at the current price to build up strength before being consumed.
...
### CPU Optimization: Depth processing pipeline (2026-03-10)
Four optimizations reducing steady-state CPU from ~100% on 4 cores to <60%:

1. **Throttle `_compute_book_metrics()` to every 5 seconds** (was every depth message = 6/s × 3 symbols). Depth diffs are still applied to the `OrderbookReconstructor` on every message — only the expensive metric computation (np.std, full bid/ask scanning, heatmap bucketing) is throttled. State: `self._book_metrics_interval` (5.0s), `self._last_book_metrics_ts` (monotonic).
2. **Bound metric computation inputs to top 1000 levels around mid** (`_METRIC_BOOK_DEPTH = 1000`). The reconstructor's internal bid/ask dicts are NOT trimmed (preserves sequence reconciliation). Bounded views are created at computation time only — in `_compute_book_metrics()` (sorted + sliced). The `OrderbookAccumulator` receives the full unsliced book because its ±10% `range_pct` heatmap needs levels across the entire range; its own internal filter is the correct bound.
3. **Remove JSON double-serialization on WS depth path.** `_wrap_message()` in all three connectors now returns `dict` (was `json.dumps → str`). `process_message()` in `full_metrics_viewer.py` accepts `dict` directly (was `json.loads(str)`). Eliminates ~18 `json.dumps`+`json.loads` round-trips per second across all streams.
4. **Batch liquidation tape log flushes.** `LiquidationTape` now flushes log/sweep files every 10 seconds OR every 50 writes (whichever first), instead of per-event. `close()` flushes unconditionally before closing file handles. Constants: `_FLUSH_INTERVAL_S = 10.0`, `_FLUSH_EVENT_COUNT = 50`.
...
| `poc/liq_heatmap_v1_{SYM}.bin` | REMOVED — deleted on startup by `EmbeddedAPIState`. V1 history endpoint returns 503. | N/A | N/A |
| `poc/liq_heatmap_v2_{SYM}.bin` | Embedded API `LiquidationHeatmapBuffer` (per-symbol, written per frame via `_write_frame_to_disk()`) | Embedded API (loaded at startup for history) | No history available — empty response, non-fatal |
...
| `poc/ws_connectors.log` | `_ws_file_logger` in `ws_connectors.py` (OKX connection lifecycle, keepalive, 30s summaries, errors) | Manual inspection via `tail -f` | No log — non-fatal |
| `poc/liq_debug.jsonl` | `EntryInference` (shared/legacy debug, not per-symbol) | Manual inspection | No log — non-fatal |
...
- **Per-symbol engines:** BTC, ETH, SOL each have independent `EngineInstance` (calibrator + heatmap + orderbook engines). All symbols have `has_orderbook=True`. Created via `SYMBOL_CONFIGS` dict and `_create_engine_for_symbol()` factory
...
- **Multi-exchange OI aggregation:** `oi_poller.py` polls Binance + Bybit + OKX every 5s for BTC, ETH, SOL. Aggregated per-symbol OI delivered via callback. Exposed via `/oi` API endpoint with per-exchange breakdown
...
**Sprint 5-A:**
- **`/liq_events` endpoint:** Returns individual force order events per symbol from in-memory deque (`maxlen=12000`). Supports `limit` (1-5000) and `since_ts` (ms) filtering. Each event includes `exchange` field. Cache bypassed (always fresh). Protected by `_liq_events_lock` (separate from `_display_lock` to avoid contention).
- **Visibility floor:** Display-only post-normalization lift for buckets crushed by p99 scaling. Prevents high-USD zones from becoming invisible in the u8 intensity grid. Per-symbol USD thresholds: `VISIBILITY_FLOOR_THRESHOLD = {"BTC": 25000, "ETH": 10000, "SOL": 5000}`. Minimum u8 intensity: `VISIBILITY_FLOOR_MIN_U8 = 25` (~10% of 255). Applied in V1/V2 snapshot grid construction in `full_metrics_viewer.py`.
- **Grid tear fix:** Index-based grid construction applied consistently across snapshot writes (prevents floating-point drift on non-integer steps like SOL `step=0.1`).
- **OB bucket sizing fix:** Orderbook endpoints enforce `MAX_GRID_BUCKETS = 10000` to prevent DoS.

**Sprint 5-B:**
- **`/market_data` endpoint:** Consolidated market data per symbol — mark price, last price, funding rate, next funding timestamp, total OI with per-exchange breakdown.
- **`/trader_ratios` endpoint:** Binance trader ratio data (BTC only, returns 400 for other symbols). Top trader account/position ratios + global long/short account ratio.
- **`/oi` delta extension:** Response now includes `previous_oi`, `oi_delta`, `oi_delta_pct`, and `oi_cvd_utc` fields. `oi_cvd_utc` is the cumulative OI delta in USD since midnight UTC, computed from the `_oi_history` deque: `sum((oi[i] - oi[i-1]) * current_price)` for consecutive readings today. Resets to 0 at midnight UTC daily. Returns 0.0 if no price or history data is available.
- **V2 pool `source` field:** Each pool in `/liq_heatmap_v2` response includes `source: "tape"|"inference"|"combined"` indicating which data source(s) contributed to the cluster.
- **`/health` exchange quality:** Health endpoint now includes per-exchange connection status and uptime information.

**OKX WebSocket fix:**
- **Text-level ping/pong keepalive:** Replaced non-functional protocol-level pings with OKX-required text-level `"ping"`/`"pong"` messages. Concurrent `_okx_keepalive()` task sends text `"ping"` after 15 seconds idle.
- **Dedicated file logger:** `_ws_file_logger` writes to `poc/ws_connectors.log` (append mode, `propagate=False`). Covers: connection establishment, subscription confirmation/error, keepalive activity, 30-second data flow summaries, disconnections with close codes, exceptions with tracebacks.

## Testing Checklist Before Deployment

- [x] All WebSocket streams connecting — Binance (depth ×3, aggTrade ×3, forceOrder, markPrice ×3), Bybit (allLiquidation ×3), OKX (liquidation-orders) — verified Mar 9 overnight run (15.8h)
- [x] Multi-exchange OI poller returning data for BTC, ETH, SOL (check `/oi?symbol=BTC`) — verified Mar 9
- [x] REST pollers returning data (ratios) — verified Mar 9
- [x] Per-symbol V2 snapshot files written for all 3 symbols (verified Mar 9 — BTC 30KB, ETH 19KB, SOL 9.4KB)
- [x] ETH snapshot contains real heatmap data with non-zero intensity arrays (verified Mar 9 — 2201 force orders, 358 inferences)
- [x] Per-symbol V1 snapshot files written for all 3 symbols (verified Mar 9 — BTC 23KB, ETH 13KB, SOL 5.5KB)
- [x] EngineManager has registered engines for all 3 symbols — verified Mar 9
- [x] Calibrator `total_events` incrementing after forceOrder events arrive — verified Mar 9 (BTC: 2543, ETH: 2199, SOL: 893 events logged)
- [x] Calibrator `self.snapshots` dict is non-empty — verified Mar 9 (56 calibration cycles per symbol)
- [x] All API endpoints return populated data — verified Mar 9 from snapshot files
- [x] OKX WebSocket stable overnight (98.9% uptime, 5 disconnects, all auto-recovered) — verified Mar 9
- [x] Per-symbol zone persistence files written (BTC 35KB, ETH 41KB, SOL 29KB active zones) — verified Mar 9
- [x] Heatmap normalization balanced (94-96% non-zero heatmap_balance entries) — verified Mar 9
- [x] V2 pool `source` field present on all pools — verified Mar 9
- [ ] Curl `/liq_events?symbol=BTC` returns events with `exchange` field (verify on next run)
- [ ] Curl `/market_data?symbol=BTC` returns populated response (verify on next run)
- [ ] Curl `/trader_ratios?symbol=BTC` returns populated response (verify on next run)
- [ ] Curl `/health` shows exchange status and uptime (verify on next run)
- [x] OB heatmap frames accumulating in `ob_heatmap_30s.bin` (BTC 6.0MB), `ob_heatmap_30s_ETH.bin` (5.9MB), `ob_heatmap_30s_SOL.bin` (6.0MB) — verified Mar 9
- [x] No silent exceptions in logs — verified Mar 9 (zero calibrator errors, zero uncaught exceptions)
- [x] Weight file being written/updated after calibration window — verified Mar 9 (56 calibration cycles per symbol)
...
| API hardening cleanup (Task 19) | PRESENT | `_resolve_symbol` replaced with `_validate_symbol` in V3 zones, per-symbol heatmap log files (`liq_tape_{SYM}.jsonl` etc.), legacy snapshot writes removed. OB endpoints now support all symbols (S4-6 superseded Task 19 BTC-only restriction). |
